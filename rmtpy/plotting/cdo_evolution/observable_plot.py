...
class ObservablePlot(Plot):
    ...
    # Plot file name
    plot_filename: str = "observable_plot"
    ...
    def set_derived_attributes(self) -> None:
        ...
        if self.xlim is None:
            self.xlim = (self.tick_times[0] / np.sqrt(dim), self.tick_times[-1])
    ...
    def plot(self):
        ...
        a = self.data["obs_eigvals"]
        A = self.data["obs_expectation"]
        A2 = self.data["obs_second_moment"]
        ...
        upper_bounds = np.minimum(1, A + a_std)
        lower_bounds = np.maximum(-1, A - a_std)
